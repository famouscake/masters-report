...
\begin{proof}
    ...
    That path must go through two of the children of $s$. Let those children be $u$ and $v$. We must have that the value of $h(u) + \max\limits_{t \in L_u}(w_{s, t}(u))$ is maximum amongst all children of $s$. Otherwise we could pick a bigger one to create a longer w-path with $v$.

    But if $u \text { and } v$ are such vertices then they would have been computed by our algorithm and the maximum w-path that passes through $s$ would be $t$ kinks.
