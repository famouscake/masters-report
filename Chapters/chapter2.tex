...
The space and time complexity of BSF are linear \cite{intro-to-algo} and therefore the space and time complexity of this algorithm are linear as well. This follows from the fact that the algorithm consists of running BFS two consecutive times. For the implementational details we refer the reader to \cite{2bfs-implementation}.

The second approach is based on the Dynamic Programming paradigm \cite{dp-poof}. Dynamic programming is a method that is used to solve optimisation problems whose solution can be reformulated recursively through solutions of subproblems of the original problem. The key ingredients in developing a dynamic programming algorithm are \cite{intro-to-algo}:
