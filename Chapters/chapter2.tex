...
We will assume that the reader has some familiarity with the basic concepts of graph theory. We will however define precisely the notation we will be using. For a graph $G = (V, E)$ we will use $V(G)$ for the vertices of $G$ and $E(G)$ for the edges of $G$. We will use the notation $N(u)$ for he neighbouhood of $u$ or all the vertices $u$ is adjacent to.

Throughout this dissertation most of the graphs we will be using will be trees. A tree is a connected graph that has no cycles. We will typically refer to trees as $T = (V, E)$. For our intents and purposes we shall define a subtree of a tree as a connected subgraph of a tree. We will refer to subtrees of rooted trees by their root. If $T$ is a rooted tree and $u$ is a node in $T$ then $T_u$ is the subtree of $T$ whoose root is $u$. In this notation if $s$ is the root of $T$ then $T_s = T$. If $u$ is any vertex that is not the root of $T$ than $T_u$ as the maximal (vertex-wise) subgraph of $T$ that contains $u$ but does not contain the parent of $u$.
...
In Chapter [] we will be interested in finding the longest path in a tree. This is known as the diameter of a tree. We will now describe two of the most popular linear time tree diameter algorithms.
...
Trees naturally exhibit optimal substructure through their subtrees. If we take any tree $T$ and root it any any of its vertices then the optimal structures of the solution is defined via all the rooted subtrees of $T$ or $\{T_u\}_{u \in V(G)}$. We can recursively define the value of the optimal solution with the following observation. Starting at the root of the tree the longest path in tree either goes through the root or is entirely contained in one of the subtrees rooted at the children of the root.

In defining this via the subtrees ot $T$. The optimal solution is either entirely contained in $T_u$ where $u$ is a child of the root of $T$ or is a combination of

In order to formalise this idea we will make use of two functions. Let $h(u)$ be the height of the subtree rooted at $u$. The height is defined as the longest path in $T_u$ from $u$ to one of the leaves of $T_u$. We will also call sight a path a height path. We will also define $D(u)$ as longest path contained entirely in $T_u$. Let the root of $T$ be $s$. The optimal solution is either entirely contained in $T_u$ where $u$ is a child of $s$ or is in combining two the height paths of two of the children of $u$. This is sumamrised by he following formula.

$$ D(s) = max\bigg\{ \max\limits_{u \in N(s)}\bigg(D(u)\bigg), \max\limits_{\substack{u, w \in N(s) \\ u \ne w}} \bigg(h(u) + h(w) + 2\bigg) , \max\limits_{u \in N(s)}\bigg(h(u)\bigg)  \bigg\} $$

where the last term is given in the case where $v$ has exactly on child. The base case for this recursive formula is at the leaves of $T$. If $u$ is a leaf of $T$ then $V(T_u) = \{u\}$. This allows us to set $h(u) = 0$ and $D(u) = 0$ and consider all leaves as base cases for the recursive formula. This algorithm can be implemented in linear time using Depth First Search (DFS) by using two auxiliary arrays that hold the values for $h(u)$ and $D(u)$ for every $u \in V(T)$ [].
